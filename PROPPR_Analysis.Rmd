...
```{r import_df2, eval=TRUE, echo=FALSE}
# import timepoint 0
df = read.xlsx("PROPPR_longitudinal_data_dictionary_edm_5.13.20.xlsx", sheet = "timepoint_2")
# PREPROCESSING
biomarker_cols <- df[51:93] #df0[8:50] 
# biomarker_cols = df0[8:50]
biomarkers = colnames(biomarker_cols)
...
for (x in biomarkers) {
  if (sum(is.na(df[x])) / nrow(df[x]) > 0.2) {
    df[x] <- NULL
  }
  if (sum(is.na(df_combined[x])) / nrow(df_combined[x]) > 0.2) {
    df_combined[x] <- NULL
...
```{r pca_df2, eval=TRUE, echo=FALSE}
combined_pca = PCA(df_combined_std, graph = FALSE, ncp = 25)
combined_eig_val <- get_eigenvalue(combined_pca)
print(combined_eig_val) # 15 dimensions account for about 81% of variance.
fviz_eig(combined_pca, addlabels = TRUE, ncp = 25)
...
```{r import_df4, eval=TRUE, echo=FALSE}
# import timepoint 0
df = read.xlsx("PROPPR_longitudinal_data_dictionary_edm_5.13.20.xlsx", sheet = "timepoint_4")
# PREPROCESSING
biomarker_cols <- df[51:93] #df0[8:50] 
# biomarker_cols = df0[8:50]
biomarkers = colnames(biomarker_cols)
...
```{r pca_df4, eval=TRUE, echo=FALSE}
combined_pca = PCA(df_combined_std, graph = FALSE, ncp = 25)
combined_eig_val <- get_eigenvalue(combined_pca)
print(combined_eig_val) # 15 dimensions account for about 81% of variance.
fviz_eig(combined_pca, addlabels = TRUE, ncp = 25)
...
```{r import_df6, eval=TRUE, echo=FALSE}
# import timepoint 0
df = read.xlsx("PROPPR_longitudinal_data_dictionary_edm_5.13.20.xlsx", sheet = "timepoint_6")
# PREPROCESSING
biomarker_cols <- df[51:93] #df0[8:50] 
# biomarker_cols = df0[8:50]
biomarkers = colnames(biomarker_cols)
...
```{r pca_df6, eval=TRUE, echo=FALSE}
combined_pca = PCA(df_combined_std, graph = FALSE, ncp = 25)
combined_eig_val <- get_eigenvalue(combined_pca)
print(combined_eig_val) # 15 dimensions account for about 81% of variance.
fviz_eig(combined_pca, addlabels = TRUE, ncp = 25)
